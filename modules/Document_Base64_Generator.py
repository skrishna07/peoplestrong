...
# Only 6 ERP-required documents
Peoplestrong_to_erp_required_documents = [
    "Passport-scan",
...
def get_candidate_document_links(candidate_folder_name: str):
    """
    Fetch only the 6 required ERP documents from SFTP, convert to Base64, and map them to ERP keys.
    ...
    try:
        ...
        # 4️⃣ Load mapping rules and filter only required documents
        file_mapping_rules_full = load_file_mapping_config()
        file_mapping_rules = {k: v for k, v in file_mapping_rules_full.items() if k in Peoplestrong_to_erp_required_documents}
        logging.info(f"[INFO] Loaded {len(file_mapping_rules)} ERP mapping rules (filtered to required 6)")
